          ...
          */}
          <script dangerouslySetInnerHTML={{__html: `
            (function(){
              ...
              if(contact) contact.addEventListener('click', function(e){ e.preventDefault(); window.dispatchEvent(new CustomEvent('setPageState', { detail: { state: 'contact' } })); clearSelected(); contact.classList.add('selected'); });
              // keep nav selection synced with external setPageState events
              window.addEventListener('setPageState', function(ev){ var s = ev?.detail?.state; clearSelected(); if(s === 'about' && about) about.classList.add('selected'); else if(s === 'portfolio' && portfolio) portfolio.classList.add('selected'); else if(s === 'projects' && projects) projects.classList.add('selected'); else if(s === 'resume' && resume) resume.classList.add('selected'); else if(s === 'contact' && contact) contact.classList.add('selected'); });
            })();
          `}} />
